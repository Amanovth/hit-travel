...
class User(AbstractUser):
    ...
    verification_code_time = models.DateTimeField(
        _("Verification code created time"), null=True, blank=True
    )
    password_reset_token = models.CharField(
        _("Password Reset"), max_length=100, blank=True, null=True, unique=True
    )
    ...
    passport_id = models.CharField(
        _("Загран паспорт"), max_length=8, null=True, blank=True, unique=True
    )
    ...
    bcard_number = models.CharField(
        _("Номер бонусного счёта"), max_length=255, null=True, blank=True
    )
    ...
    first_name_en = models.CharField(
        _("Имя на латинице"), max_length=150, null=True, blank=True
    )
    last_name_en = models.CharField(
        _("Фамилия на латинице"), max_length=150, null=True, blank=True
    )
...
class BonusHistory(models.Model):
    CURRENCY_CHOICES = (("СОМ", "СОМ"), ("USD", "USD"))

    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="bonus_history"
    )
    ...
    currency = models.CharField(
        _("Валюта"), choices=CURRENCY_CHOICES, max_length=5, default="СОМ"
    )
...
class RequestTour(models.Model):
    ...
    user = models.ForeignKey(
        User, verbose_name=_("Клиент"), on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    operatorlink = models.URLField(
        _("Ссылка на оператора"), max_length=1000, null=True, blank=True
    )
    ...
    bonuses = models.DecimalField(
        _("Бонусы"), default=0, max_digits=10, decimal_places=2, null=True, blank=True
    )
    ...
    agreement = models.FileField(
        _("Договор"), upload_to="agreements", null=True, blank=True
    )
...
class Document(models.Model):
    request = models.ForeignKey(
        RequestTour, on_delete=models.CASCADE, related_name="documents"
    )
    name = models.CharField(
        _("Название документа"), max_length=255, null=True, blank=True
    )
...
class Traveler(models.Model):
    ...
    main = models.ForeignKey(
        RequestTour, on_delete=models.CASCADE, related_name="travelers"
    )
    dateofborn = models.DateField(_("Дата рождения"), null=True, blank=True)
    first_name = models.CharField(_("Имя"), max_length=100)
    last_name = models.CharField(_("Фамилия"), max_length=100)
    gender = models.CharField(
        _("Пол"), choices=GENDER_CHOICES, max_length=3, null=True, blank=True
    )
